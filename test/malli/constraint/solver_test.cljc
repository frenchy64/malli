...
(deftest -conj-solutions-test
  (is (= (mcs/-conj-solutions)
         [{}]))
  (is (= (mcs/-conj-solutions [{:order [:a], :present {:a true}}])
         [{:order [:a], :present {:a true}}]))
  (is (= (mcs/-conj-solutions [] [{:order [:a], :present {:a true}}])
         []))
  (is (= (mcs/-conj-solutions [{:order [:a], :present {:a true}}] [{:order [:a], :present {:a true}}])
         [{:order [:a], :present {:a true}}]))
  (is (= (mcs/-conj-solutions '({:order [:a], :present {:a true}})
                              '({:order [:b], :present {:b false}}))
         [{:order [:a :b] :present {:a true :b false}}]))
  (is (= (mcs/-conj-solutions '({:< 2})
                              '({:> 0}))
         [{:> 0 :< 2}]))
  (is (= (mcs/-conj-solutions '({:< 2})
                              '({:< 0}))
