...
(deftest not-humanize-test
  ...
  (is (= ["should not be a double"] (me/humanize (m/explain [:not :double] 1.0))))
  (is (= ["should not be at least 1.0"] (me/humanize (m/explain [:not [:double {:min 1.0}]] 1.0))))
  (is (= ["should not be at most 1.0"] (me/humanize (m/explain [:not [:double {:max 1.0}]] 1.0))))
  (is (= ["should not be 1.0"] (me/humanize (m/explain [:not [:double {:min 1.0 :max 1.0}]] 1.0))))
  (is (= ["should not be a boolean"] (me/humanize (m/explain [:not :boolean] true))))
  (is (= ["should not be a keyword"] (me/humanize (m/explain [:not :keyword] :a))))
  (is (= ["should not be a symbol"] (me/humanize (m/explain [:not :symbol] 'a))))
  (is (= ["should not be a qualified keyword"] (me/humanize (m/explain [:not :qualified-keyword] ::a))))
  (is (= ["should not be a qualified symbol"] (me/humanize (m/explain [:not :qualified-symbol] `a))))
  (is (= ["should not be a uuid"] (me/humanize (m/explain [:not :uuid] (random-uuid)))))
  (is (= ["should not be larger than 1"] (me/humanize (m/explain [:not [:> 1]] 2))))
  (is (= ["should not be at least 1"] (me/humanize (m/explain [:not [:>= 1]] 2))))
  (is (= ["should not be smaller than 1"] (me/humanize (m/explain [:not [:< 1]] 0))))
  (is (= ["should not be at most 1"] (me/humanize (m/explain [:not [:<= 1]] 0))))
