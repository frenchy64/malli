...
A polymorphic function using `m/all` is generatively tested by instantiating schema variables with small schemas or `:any`.
...
```clojure
[:=> [:cat :nil] :nil]
[:=> [:cat [:enum 50 "foo" :bar]] [:enum 50 "foo" :bar]]
[:=> [:cat :any] :any]
```
...
A polymorphic function will be instrumented as if all its schema variables were replaced by `:any`.
